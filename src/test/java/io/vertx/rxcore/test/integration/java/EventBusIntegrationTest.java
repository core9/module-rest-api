...
public class EventBusIntegrationTest extends TestVerticle {
	...
	@Test
	// Send some messages in series - i.e. wait for result of previous one
	...
	public void testSimpleSerial() {
		...
		Observable<RxMessage<String>> obs1 = rxEventBus.observeSend("foo",
				"ping!");
		Observable<RxMessage<String>> obs2 = rxEventBus.observeSend("foo",
				"ping!");
		Observable<RxMessage<String>> obs3 = rxEventBus.observeSend("foo",
				"ping!");

		Observable<RxMessage<String>> concatenated = Observable.concat(obs1,
				obs2, obs3);
	...
	@Test
	// Send some messages in series where next message sent is function of reply
	...
	public void testSerial() {
		...
		Observable<RxMessage<String>> obs2 = obs1
				.flatMap(new Func1<RxMessage<String>, Observable<RxMessage<String>>>() {
					@Override
					public Observable<RxMessage<String>> call(
							RxMessage<String> reply) {
						return rxEventBus.send("foo", reply.body() + "ping2");
					}
				});
		Observable<RxMessage<String>> obs3 = obs2
				.flatMap(new Func1<RxMessage<String>, Observable<RxMessage<String>>>() {
					@Override
					public Observable<RxMessage<String>> call(
							RxMessage<String> reply) {
	...
	@Test
	// Send some messages in parallel and wait for all replies before doing
	// something
	...
	@Test
	// Send some messages in parallel and return result of concatenating all the
	// messages
	public void testConcatResults() {
		...
		RxAssert.assertSequenceThenComplete(result.takeLast(1),
				"pongApongBpongC");
	}
	...
	public void testReplyToReply() {
		...
		Observable<RxMessage<String>> obsReply1 = rxEventBus
				.registerHandler("foo");

		Observable<RxMessage<String>> obsReply2 = obsReply1
				.flatMap(new Func1<RxMessage<String>, Observable<RxMessage<String>>>() {
					@Override
					public Observable<RxMessage<String>> call(
							RxMessage<String> stringRxMessage) {
		...
		Observable<RxMessage<String>> obsReply3 = obsReply2
				.flatMap(new Func1<RxMessage<String>, Observable<RxMessage<String>>>() {
					@Override
					public Observable<RxMessage<String>> call(
							RxMessage<String> stringRxMessage) {
		...
		Observable<RxMessage<String>> obsSend1 = rxEventBus.send("foo",
				"hello1");

		Observable<RxMessage<String>> obsSend2 = obsSend1
				.flatMap(new Func1<RxMessage<String>, Observable<RxMessage<String>>>() {
					@Override
					public Observable<RxMessage<String>> call(
							RxMessage<String> stringRxMessage) {
	...
	public void testRetry() {
		...
		vertx.eventBus().registerHandler("every3",
				new Handler<Message<Integer>>() {
		...
		// Keep asking
		Observable<String> res = rx
				.<String, String> observeSend("every3", "please")
	...
	public void testStream() {
		...
		vertx.eventBus().registerHandler("countdown",
				new Handler<Message<Integer>>() {

					public void sendBatch(final Message<Integer> msg, int from,
							int length) {
		...
		Observable<Buffer> res = rx
				.<Integer, JsonArray> observeStream("countdown", 400)
	...
	public void testFlow() {
		...
		Observable<RxStream<Integer, JsonArray>> res = rx
				.<Integer, JsonArray> observeStream("countdown", 4000)
